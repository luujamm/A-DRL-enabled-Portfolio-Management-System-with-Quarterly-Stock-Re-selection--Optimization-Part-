...
def obs_normalizer(observation):
    ...
    observation = observation[:, 1:, :4] / observation[:, -1:, -1:] # normalize to last close
    ...
    #norm = np.linalg.norm(observation, axis=1, keepdims=True) + 1e-8
...
def date_to_index(date_string, dating):
    ...
    # print(date_idx[0])
    # if date_idx == None:
    #     print(date_string + ' is a holiday')
    return date_idx[0][0]  

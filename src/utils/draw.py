...
def show_train_results(args, agent, path):
    ...
    draw_train_figs(args, agent, path)
...
def draw_test_figs(args, recorder, target_stocks, test_num):
    ...
    fmt_month = mdates.DayLocator(bymonthday=1)
...
def show_val_results(args, agent, recorder, target_stocks, test_num, iteration, model_fn, path):
    ...
    if len(agent.val_reward) >= t and mean_reward >= np.max(agent.val_reward[t - 1:]):
        agent.save(model_fn)

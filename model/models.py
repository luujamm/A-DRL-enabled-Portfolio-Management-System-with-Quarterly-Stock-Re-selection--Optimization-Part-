    ...
    def __init__(self, args, day_length, action_dim):
        ...
        self.Conv1 = nn.Sequential(
            nn.Conv2d(in_channels=self.in_ch1, out_channels=self.out_ch1,
                      ...
                      padding=(0, int((self.cvks1-1)/2)), bias=self.bias),
            nn.LayerNorm(normalized_shape=[self.out_ch1, self.action_dim, day_length], elementwise_affine=self.affine),
            nn.ReLU()
        )
        ...
        self.Conv2 = nn.Sequential(         # input shape (, ,  )
            nn.Conv2d(in_channels=self.out_ch1, out_channels=self.out_ch2,
                      kernel_size=(1, self.cvks2), stride=1, bias=self.bias),
            nn.LayerNorm(normalized_shape=[self.out_ch2, self.action_dim, 1], elementwise_affine=self.affine),
            nn.ReLU()
        )
        ...
        self.Conv3 = nn.Sequential(         # input shape ( ,  ,  )
            ...
            nn.LayerNorm(normalized_shape=[self.out_ch3, self.action_dim, 1], elementwise_affine=self.affine),
            nn.ReLU()
        )
...
class CNN_tcn(nn.Module):  # Independent Identical Evaluators
    ...
    def __init__(self, args, day_length, action_dim):
        ...
        self.Conv = nn.Sequential(         # input shape ( ,  ,  )
    ...
    def forward(self, s, w):
        ...
        #x = torch.cat((x1, x2, s + x1 + x2 + x3), 1) # concate
        x = self.Conv(x) # 1x1
        x = self.Conv2(x)
